...
for sub=1:length(allsub)
    ...
    for i=1:length(decision_trial)
        if decision_trial(i).subres==1 %选择确定
            ...
            decision_trial(i).REGR=diff([decision_trial(i).certainty, max(decision_trial(i).uncertainty)]);
        else %选择不确定
            ...
            if decision_trial(i).this_score == min(decision_trial(i).uncertainty)
                decision_trial(i).REGR=diff([decision_trial(i).certainty, decision_trial(i).this_score]);
            else
                 decision_trial(i).REGR=0;
            end
        end
    end
    ...
    bounds=[-10,-10,-10,-10,-10,0;10,10,10,10,10,1];
    %w0 w1 w2 w3 gamma
    record=[];
    parfor i=1:5
